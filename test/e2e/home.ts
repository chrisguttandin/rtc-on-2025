...
test('should display the correct headline', async () => {
    await expect(home.getHeadline()).toHaveText('Designing a media container library for the web');
});
...
test('should go to the next slide when using the arrow key', async ({ page }) => {
    ...
    await expect(home.getSubHeadline()).toHaveText("Cześć,jestem Christoph Guttandin");
});
